...
bool IsTritonSupportedConversion(PrimitiveType output, PrimitiveType input,
                                 const se::GpuComputeCapability& gpu_version) {
  if ((input == PrimitiveType::F8E4M3FN || output == PrimitiveType::F8E4M3FN) &&
  ...
  if (IsTritonSupportedDataType(output, gpu_version) &&
      IsTritonSupportedDataType(input, gpu_version)) {
    return true;
  }

  if (output == PrimitiveType::S4) {
    return true;
  }
...
// operate on "unwrapped" values (scalars) and that code gen supports a
// different set of operations.
//
// In principle `operand_is_scalar` can be added also to the functions that
// check support for unary and ternary ops, but there was no need to do this so
// far.
