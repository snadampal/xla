...
using ConvertTest = TritonSupportTestWithParam;

TEST_P(ConvertTest, ConvertFromF32) {
  auto [data_type, opcode, cc] = GetParam();
  const std::string kHloTemplate = R"(
ENTRY triton_computation {
  parameter_0 = f32[33,68]{1,0} parameter(0)
  ROOT convert = $0[33,68]{1,0} $1(parameter_0)
})";

  TF_ASSERT_OK_AND_ASSIGN(
      TestedInstruction ti,
      ParseTemplateAndGetInstruction(kHloTemplate, data_type, opcode));
...
TEST_P(ConvertTest, ConvertToF32) {
  auto [data_type, opcode, cc] = GetParam();
  const std::string kHloTemplate = R"(
ENTRY triton_computation {
  parameter_0 = $0[33,68]{1,0} parameter(0)
  ROOT convert = f32[33,68]{1,0} $1(parameter_0)
})";
...
INSTANTIATE_TEST_SUITE_P(ConvertTestSuite, ConvertTest,
                         AllTestCombinationsForOpcodes(kTestedOpsConvert),
                         TritonSupportTestTypeOpcodeAndDeviceToString);
...
absl::flat_hash_set<HloOpcode> AllTestedOpcodes() {
  ...
  absl::flat_hash_set<HloOpcode> ret{HloOpcode::kParameter};
