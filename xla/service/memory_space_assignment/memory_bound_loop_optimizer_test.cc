...
class LoopOptimizerBestFitHeapTest : public ::testing::Test {
 public:
  ...
  bool IsAllocateBetweenSuccessful(int64_t begin_idx, int64_t end_idx,
                                   int64_t size) {
    EvenOddChunkPair chunks = heap_.AllocateBetween(begin_idx, end_idx, size);
    return chunks.first.has_value() && chunks.second.has_value();
  }

  bool CanFindAllocationBetween(int64_t begin_idx, int64_t end_idx,
                                int64_t size) {
    EvenOddChunkPair chunks =
        heap_.FindAllocationBetween(begin_idx, end_idx, size);
    return chunks.first.has_value() && chunks.second.has_value();
  }
...
TEST_F(LoopOptimizerBestFitHeapTest, TestAllocateBetween) {
  EXPECT_TRUE(IsAllocateBetweenSuccessful(3, 8, 16));
  EXPECT_TRUE(IsAllocateBetweenSuccessful(-3, 2, 16));
  EXPECT_TRUE(IsAllocateBetweenSuccessful(0, 2, 16));
  EXPECT_TRUE(IsAllocateBetweenSuccessful(3, 5, 16));
  EXPECT_TRUE(IsAllocateBetweenSuccessful(0, 5, 16));
  EXPECT_FALSE(IsAllocateBetweenSuccessful(0, 5, 16));
...
TEST_F(LoopOptimizerBestFitHeapTest, TestRemoveChunk) {
  ...
  // and odd iterations.
  EXPECT_FALSE(IsAllocateBetweenSuccessful(0, 2, 16));
  ...
  EXPECT_TRUE(CanFindAllocationBetween(0, 5, 32));
  EXPECT_TRUE(CanFindAllocationBetween(-1, 4, 32));
  EXPECT_TRUE(CanFindAllocationBetween(2, 7, 32));
